...
func New(ctx context.Context, cfg Config, opts ...func(*tessera.StorageOptions)) (*Storage, error) {
	...
	// TODO: GCS was using a JSON Reads. Do we want to do the same? I rememvber looking in to this earlier
...
func (s *Storage) setTile(ctx context.Context, level, index, logSize uint64, tile *api.HashTile) error {
	...
	r := string('*') // string expected by IfNoneMatch
	return s.objStore.setObject(ctx, tPath, data, &s3.PutObjectInput{IfNoneMatch: &r}, logContType)
}
...
func (s *Storage) setEntryBundle(ctx context.Context, bundleIndex uint64, logSize uint64, bundleRaw []byte) error {
	...
	// passed in here.
	r := string('*') // string expected by IfNoneMatch
	if err := s.objStore.setObject(ctx, objName, bundleRaw, &s3.PutObjectInput{IfNoneMatch: &r}, logContType); err != nil {
...
func (s *s3Storage) setObject(ctx context.Context, objName string, data []byte, cond *s3.PutObjectInput, contType string) error {
	...
	if _, err := s.s3Client.PutObject(ctx, put); err != nil {
		...
		if ee, ok := err.(awserr.Error); ok && ee.Code() == "PreconditionFailed" {
