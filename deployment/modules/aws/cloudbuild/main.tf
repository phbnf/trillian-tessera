...
locals {
  ...
  conformance_aws_docker_image = "${local.artifact_repo}/conformance-aws"
}

data "google_secret_manager_secret_version" "aws_acces" {
 secret   = "phboneff-aws-gcpcloudbuild-access"
}

data "google_secret_manager_secret_version" "aws_secret" {
 secret   = "phboneff-aws-gcpcloudbuild-secret"
}
  ...
  build {
   ...
   # }
    ...
    step {
      ...
      args = [
        "build",
        "-t", "${local.conformance_aws_docker_image}:$SHORT_SHA",
        "-t", "${local.conformance_aws_docker_image}:latest",
        "-f", "./cmd/conformance/aws/Dockerfile",
        "."
      ]
    }
    ## Push the image.
    step {
      id   = "docker_push_conformance_aws"
      name = "gcr.io/cloud-builders/docker"
      args = [
        "push",
        "--all-tags",
        local.conformance_aws_docker_image
      ]
      wait_for = ["docker_build_conformance_aws"]
    }
    step {
      id       = "generate_keys"
      name     = "golang"
    ...
    step {
      ...
      EOT
      dir    = "deployment/live/aws/conformance/ci"
      env = [
        "TF_IN_AUTOMATION=1",
        "TF_INPUT=false",
        "TF_VAR_project_id=${var.project_id}"
      ]
      wait_for = ["docker_push_conformance_aws", "generate_keys"]
    }
    ...
    step {
      ...
      env = [
        "AWS_ACCESS_KEY_ID=${data.google_secret_manager_secret_version.aws_acces.secret_data}",
        "AWS_SECRET_ACCESS_KEY=${data.google_secret_manager_secret_version.aws_secret.secret_data}",
