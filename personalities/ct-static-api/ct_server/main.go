...
func main() {
	...
	cfg, err = ctfe.LogMultiConfigFromFile(*logConfig)

	if err != nil {
		klog.Exitf("Failed to read config: %v", err)
	}

	err = ctfe.ValidateLogMultiConfig(cfg)
	if err != nil {
	...
	for _, c := range cfg.Config {
		inst, err := setupAndRegister(ctx, c, *tesseraDeadline, corsMux, *maskInternalErrors)
		if err != nil {
...
func setupAndRegister(ctx context.Context, cfg *configpb.LogConfig, deadline time.Duration, mux *http.ServeMux, maskInternalErrors bool) (*ctfe.Instance, error) {
	vCfg, err := ctfe.ValidateLogConfig(cfg)
	if err != nil {
		return nil, err
	}
	var storage tessera.Storage
	switch cfg.StorageConfig.Config.(type) {
	case *configpb.StorageConfig_Gcp:
		gcpCfg := gcp.Config{
			// TODO(phboneff): get projectID in a better way
			ProjectID: "phboneff-dev",
			Bucket:    cfg.StorageConfig.GetGcp().Bucket,
			Spanner:   cfg.StorageConfig.GetGcp().SpannerDbPath,
		}
	...
	store := tessera.NewCertificateTransparencySequencedWriter(storage)
