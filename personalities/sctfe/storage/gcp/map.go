...
func (s GCSStorage) Exists(ctx context.Context, key [32]byte) (bool, error) {
	...
	klog.V(2).Infof("Exists: object %q already exists", objName)
	return true, nil
}
...
func (s GCSStorage) Get(ctx context.Context, key [32]byte) ([]byte, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("getObject: failed to create reader for object %q in bucket %q: %w", objName, s.bucket.BucketName(), err)
	}
...
func (s GCSStorage) Add(ctx context.Context, key [32]byte, data []byte) error {
	...
	if err := w.Close(); err != nil {
		...
		if ee, ok := err.(*googleapi.Error); ok && ee.Code == http.StatusPreconditionFailed {
			klog.V(2).Infof("Add: object %q already exists, continuing", objName)
			return nil
		}
